...
(function ($) {
    ...
    $.fn.validatebox.defaults = { required: false, validType: null, validParams: null, delay: 200, missingMessage: "This field is required.", invalidMessage: null, tipPosition: "right", deltaX: 0, novalidate: false, tipOptions: { showEvent: "none", hideEvent: "none", showDelay: 0, hideDelay: 0, zIndex: "", onShow: function () {
     ...
     pcphm: { validator: function (value1, param1) {
        return /^[\u4e00-\u9fa5][A-Za-z]$/.test(value1);
    }, message: "格式应为1位汉字+1位字母"
    }, 
     scphm: { validator: function (value2, param2) {
        return /^[A-Za-z0-9]{4}[A-Za-z0-9\u4e00-\u9fa5]$/.test(value2);
    }, message: "格式应为4位数字或字母+1位数字或字母或汉字"
    }, 
     clsbdh: { validator: function (value3, param3) {
        return /^[A-Za-z0-9]{17}$/.test(value3);
    }, message: "格式应为17位英文和数字的组合"
    }, 
     phone: { validator: function (value4, param4) {
        return /^((\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)$/.test(value4);
    }, message: "格式应为11位数字或区号-号码或区号-号码-分机号"
    }, 
     email: { validator: function (_442) {
